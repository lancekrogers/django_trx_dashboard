...
def test_portfolio(token):
    ...
    # Historical data
    response = requests.get(f"{BASE_URL}/v1/portfolio/history?period=24h", headers=headers)
    if response.status_code == 200:
...
def test_transactions(token):
    ...
    # List transactions
    response = requests.get(f"{BASE_URL}/v1/transactions/?page=1&page_size=10", headers=headers)
    if response.status_code == 200:
        data = response.json()
        transactions = data['results']
        print(f"✓ List transactions: Found {data['count']} total, showing {len(transactions)}")
...
def test_sse(token):
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/v1/portfolio/stream", 
            headers=headers,
            stream=True,
            timeout=1
        )
