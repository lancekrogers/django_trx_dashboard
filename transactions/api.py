...
def list_transactions(request, filters: TransactionFilterSchema = Query(...)):
    ...
    qs = Transaction.objects.filter(
        wallet__user=request.user, wallet__is_active=True
...
def transaction_stats(request):
    ...
    total_volume = user_transactions.aggregate(total=Sum("usd_value"))["total"] or 0

    # 24h transactions
    cutoff_24h = timezone.now() - timedelta(days=1)
    transactions_24h = user_transactions.filter(timestamp__gte=cutoff_24h).count()

    # Most traded asset
    asset_counts = (
        user_transactions.values("asset_symbol")
